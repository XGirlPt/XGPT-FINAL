...
const ModificarFotos: React.FC<ModificarFotosProps> = ({
  handleVoltar,
  open,
  onClose,
  onOpenChange,
    ...
    if (event.target.files && event.target.files.length > 0) {
      ...
      if (uploadedPhotoURLs.length > 0) {
        ...
        try {
          ...
          if (photoError) {
            console.error(
              'Erro ao inserir URLs das fotos na tabela profilephoto:',
              photoError.message
            );
          ...
          console.log(
            'URLs das fotos inseridas com sucesso na tabela profilephoto:',
            photoData
          );
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl h-2/3 md:h-4/5 sm:max-h-[80vh] p-0 overflow-hidden">
        <DialogHeader className="bg-pink-800 py-6">
          <DialogTitle className="text-3xl font-bold tracking-wide text-center">
            Gerir Fotos
          </DialogTitle>
          <h2 className="text-center text-gray-200 text- md:text-sm mt-2">
