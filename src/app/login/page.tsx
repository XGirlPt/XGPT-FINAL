...
const Login = () => {
  ...
  useEffect(() => {
    ...
    if (token && email) {
      ...
      console.log(
        'Token ou email ausente no localStorage, usuário não está logado.'
      );
    }
  }, [dispatch]);
  ...
  const handleLogin = async () => {
    ...
    if (error) {
      ...
      if (user) {
        ...
        if (!userUID) {
          setErrorMessage(
            'Erro: não foi possível recuperar os dados do usuário.'
          );
          return;
        }
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Connectez-vous à X-Girl pour découvrir des expériences uniques et confidentielles."
        />
        <meta
          name="keywords"
          content="connexion, X-Girl, login, expérience confidentielle, sécurité"
        />
      ...
      <div className="flex flex-col items-center mt-36 h-screen pb-4 ">
        ...
        <div className=" w-full mt-10 max-w-md md:w-1/3 rounded-lg shadow-2xl shadow-gray-400 dark:shadow-gray-800 border border-gray-200 dark:border-gray-700 px-6 py-6 space-y-4">
          ...
          <div className="text-center mt-4">
            <Link
              href="/regista2"
              className="text-pink-500 cursor-pointer hover:text-pink-600 font-semibold"
            >
