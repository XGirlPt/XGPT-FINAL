...
export function RegistoContacto() {
  ...
  const handleContinue = async () => {
    try {
      ...
      // Coletar os dados selecionados
      const paymentOptions = categories
        .find((cat) => cat.id === "payment")!
        .options!.filter((opt) => opt.checked)
        .map((opt) => opt.id);
      const languageOptions = categories
        .find((cat) => cat.id === "languages")!
        .options!.filter((opt) => opt.checked)
        .map((opt) => opt.id);
      ...
      const profileData = {
        userUID,
        pagamento: paymentOptions,
        lingua: languageOptions,
        servico: serviceOptions,
        premium: isPremium,
      ...
      if (existingProfile) {
        ...
        // Inserir novo perfil (caso raro, já que o perfil deve existir desde RegisterPage)
      ...
      dispatch(updatePagamento(paymentOptions));
      dispatch(updateLingua(languageOptions));
      dispatch(updateServico(serviceOptions));
      dispatch(updatePremium(isPremium));
  ...
  const handlePlanoSelect = (plano: "free" | "premium") => {
    if (plano === "premium") {
      ...
      toast.success(
        "Plano Premium selecionado! Continue seu registro com todas as opções desbloqueadas."
      );
    }
      ...
      <div className="flex flex-col md:flex-row justify-between items-start mb-6">
        <div className="w-full md:w-auto">
          ...
          <div className="w-44 mt-4">
            {/* Placeholder para FiltroPrice, se necessário */}
          </div>
        </div>
      ...
      <Dialog open={showUpgradePopup} onOpenChange={setShowUpgradePopup}>
        <DialogContent className="sm:max-w-[900px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Você atingiu o limite de 5 serviços no plano gratuito. Escolha o plano Premium para
              desbloquear até 40 serviços e outros benefícios exclusivos! O pagamento será solicitado
              no final do registro.
            </DialogDescription>
          </DialogHeader>
          <div className="mt-4">
            <SubscriptionPlan userUID={profile.userUID || ""} onPlanoSelect={handlePlanoSelect} />
          </div>
